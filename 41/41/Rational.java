...
class Rational implements Comparable {
	...
	int otherS=((Rational)other).toString().indexOf("/");
	int otherL=((Rational)other).toString().length();
	int otherN=Integer.parseInt(((Rational)other).toString().substring(0,otherS));
	//System.out.println(otherN);
	int otherD=Integer.parseInt(((Rational)other).toString().substring(otherS+1,otherL));
